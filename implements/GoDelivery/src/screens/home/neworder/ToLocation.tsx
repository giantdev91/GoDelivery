...
const ToLocation = ({route, navigation}: {route: any; navigation: any}) => {
  ...
  const checkSystemSetting = async () => {
    Action.sysSetting.get().then(res => {
      if (res.data.data) {
  ...
  const handleConfirmButton = async () => {
    ...
    if (distance < 50) {
      Dialog.show({
        ...
        title: 'GoDelivery',
        textBody:
          'Pick up and drop off locations cannot be the same. Please choose different locations.',
        button: 'OK',
      });
  ...
  const calculatePriceByDistance = (distance: string) => {
    ...
    if (dis < 4) {
      returnVal = (
        setting && setting?.basePrice ? setting?.basePrice : 89
      ).toString();
    } else {
      returnVal = (
        (Number.parseFloat(setting?.basePrice ? setting?.basePrice : 0) * dis) /
        4
      ).toFixed(2);
    }
    return returnVal;
    ...
    <AlertNotificationRoot>
      <View style={GlobalStyles.container}>
        ...
        <View style={{flex: 1}}>
          <ViewShot
            ref={ref}
            options={{fileName: 'Your-File-Name', format: 'jpg', quality: 0.3}}
            style={{flex: 1}}>
            {region.latitude != null && (
              <MapView
                ...
                region={region}>
                <Marker coordinate={orderInfo['fromLocation']}>
                ...
                {clickedConfirm && (
                  <MapViewDirections
                    ...
                    onReady={result => {
                      ...
                      setEstimationTime(
                        `${Math.ceil(
                          result.duration + DEFAULT_ARRIVAL_TIME,
                        ).toString()}`,
                      );
          ...
          <View
            ...
            <View
              style={[
                {backgroundColor: GoDeliveryColors.white, borderRadius: 5},
              ...
              <View style={[GlobalStyles.locationStrContainer]}>
                ...
                <Text style={GlobalStyles.locationStr}>
                  {orderInfo['fromStr']}
                </Text>
              </View>
              <Divider style={styles.divider} />
              <TouchableOpacity
                onPress={toLocationSearch}
          ...
          {clickedConfirm && (
            <View style={[styles.informationPad, GlobalStyles.shadowProp]}>
              <View style={styles.informationStrContainer}>
                <Text style={GlobalStyles.headerTitle}>
                  {CommonFunctions.getLocalNumberValue(price)}
                </Text>
                <Text style={GlobalStyles.headerTitle}>{distance}KM</Text>
              </View>
              <View style={styles.informationStrContainer}>
                <Text style={GlobalStyles.textDisable}>Estimated Price</Text>
                <Text style={GlobalStyles.textDisable}>Distance</Text>
              </View>
            </View>
          )}
